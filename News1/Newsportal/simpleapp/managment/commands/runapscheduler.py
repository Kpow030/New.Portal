...
from News1.Newsportal.models.models import Post
from News1.Newsportal.News.models import Category

logger = logging.getLogger(__name__)


def my_job(categories):
    # Your job processing logic here...
    today = datetime.datetime.now()
    last_week = today - datetime.timedelta(days=7)
    posts = Post.objects.filter(date_gte=last_week)
    categories = set(posts.value_list('category__name', flat=True))
    subscribers = set(Category.objects.filters(name__in=categories).values_list('subscribers__email', flat=True))
